...
def start_mqtt_broker(server):
    ...
    server.cmd(f"echo 'listener {BROKER_PORT} 0.0.0.0' > {conf_file}")
    server.cmd(f"mosquitto -c {conf_file} -d -v")
    time.sleep(4)
...
def start_mqtt_subscriber(server):
    """Start subscriber on server and log to OUTPUT_LOG_DIR."""
    log_file = f"{OUTPUT_LOG_DIR}/subscriber.log"
    cmd = f'python3 sensor_subscriber.py sensors/# > {log_file} 2>&1 &'
    server.cmd(cmd)
...
def start_mqtt_publisher(host, sensor_name):
    ...
    host.cmd(cmd)
    info(f'✅ MQTT publisher started on {sensor_name} host, logging to {log_file}\n')
# ==============================================================
# Create network
# ==============================================================
